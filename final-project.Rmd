...
survdiff(Surv(time = survival_months, event = survival_status) ~ age)
...
```{r}
# check the proportional hazards assumption for each covariate

## age
ggsurvplot(survfit(Surv(time = survival_months, event = survival_status) ~ age), data = seer, fun="cloglog")
